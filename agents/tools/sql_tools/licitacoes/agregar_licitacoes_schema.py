"""Schemas da tool publica agregar_licitacoes."""
...
class AgregarLicitacoesParams(LicitacoesToolBaseSchema):
    ...
    def _normalize_filtros(cls, value: Any) -> LicitacoesFiltroSchema:
        if value is None:
            return LicitacoesFiltroSchema()
        if isinstance(value, LicitacoesFiltroSchema):
            return value
        if not isinstance(value, dict):
            raise ValueError("filtros deve ser um objeto")
        return LicitacoesFiltroSchema.model_validate(value)
    ...
    def _validate_aggregation(self) -> "AgregarLicitacoesParams":
...
class AgregacaoLicitacoesItem(LicitacoesToolBaseSchema):
    ...
    ano_abertura: str | None = None
    contagem: int | None = None
    soma_valor_estimado: float | None = None
    media_valor_estimado: float | None = None
