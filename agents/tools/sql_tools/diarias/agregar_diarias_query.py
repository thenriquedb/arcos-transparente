...
def agregar_diarias(
    ...
    """
    try:
        params = AgregarDiariasParams.model_validate(
            {
                "filtros": filtros,
        ...
        fallback_metadata = AgregarDiariasMetadata(
            metrica="soma_valor_pago",
            ordenar_por="metrica",
            ordem="desc",
            limite=10,
        )
        return AgregarDiariasResponse(
            total_grupos=0,
            resultados=[],
    ...
    if params.agrupar_por is None:
        ...
        return AgregarDiariasResponse(
            ...
            sugestao=(
                "Nenhuma diaria encontrada com os filtros." if not valor_total else None
            ),
        ).model_dump(mode="json")
    ...
    return AgregarDiariasResponse(
        ...
        mensagem=mensagem,
        sugestao=(
            "Nenhuma diaria encontrada com os filtros." if not resultados else None
        ),
    ).model_dump(mode="json")
