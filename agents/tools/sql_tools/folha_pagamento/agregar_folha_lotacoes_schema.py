"""Schemas da tool publica agregar_folha_lotacoes."""
...
from .consultar_folha_lotacoes_schema import FolhaLotacoesFiltroSchema


ALLOWED_FOLHA_LOTACOES_GROUP_FIELDS = {"lotacao"}
ALLOWED_FOLHA_LOTACOES_METRICS = {
    "contagem",
    "soma_salario_base",
...
class AgregarFolhaLotacoesParams(SqlToolBaseSchema):
    ...
    def _normalize_agrupar_por(cls, value: Any) -> str | None:
        ...
        if normalized is None:
            return None
        if normalized not in ALLOWED_FOLHA_LOTACOES_GROUP_FIELDS:
    ...
    def _normalize_metrica(cls, value: Any) -> str:
        ...
        if normalized not in ALLOWED_FOLHA_LOTACOES_METRICS:
    ...
    def _validate_aggregation(self) -> AgregarFolhaLotacoesParams:
...
class AgregarFolhaLotacoesMetadata(SqlToolBaseSchema):
...
class AgregacaoFolhaLotacoesItem(SqlToolBaseSchema):
...
class AgregarFolhaLotacoesResponse(SqlToolBaseSchema):
