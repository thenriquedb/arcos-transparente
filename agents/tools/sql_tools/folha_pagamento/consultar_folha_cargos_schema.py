"""Schemas da tool publica consultar_folha_cargos."""
...
ALLOWED_FOLHA_CARGOS_SORT_FIELDS = {
    "competencia_ano",
    "competencia_mes_num",
    "cargo",
    "servidor",
...
class FolhaCargosFiltroSchema(SqlToolBaseSchema):
    ...
    @field_validator("cargo", "servidor", mode="before")
    @classmethod
...
class ConsultarFolhaCargosParams(SqlToolBaseSchema):
    ...
    def _normalize_ordenar_por(cls, value: Any) -> str:
        ...
        if normalized not in ALLOWED_FOLHA_CARGOS_SORT_FIELDS:
    ...
    def _normalize_campos(cls, value: Any) -> list[str]:
        return normalize_selected_fields(
            value,
            allowed_fields=ALLOWED_FOLHA_CARGOS_FIELDS,
            require_list=False,
            error_style="campos",
        )


class ConsultarFolhaCargosMetadata(SqlToolBaseSchema):
    ...
    limite: int
    offset: int
    campos: list[str] = Field(default_factory=lambda: list(ALLOWED_FOLHA_CARGOS_FIELDS))


class ConsultarFolhaCargosResponse(SqlToolBaseSchema):
