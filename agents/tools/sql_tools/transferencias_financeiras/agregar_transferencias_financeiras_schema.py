"""Schemas da tool publica agregar_transferencias_financeiras."""
...
ALLOWED_TRANSFERENCIAS_GROUP_FIELDS = {
    "tipo_registro",
    "ano",
    "unidade_concessora",
    "unidade_recebedora",
...
ALLOWED_TRANSFERENCIAS_METRICS = {
    "contagem",
    "soma_valor",
    "soma_programacao_inicial",
}
...
class AgregarTransferenciasFinanceirasParams(SqlToolBaseSchema):
    ...
    def _normalize_agrupar_por(cls, value: Any) -> str | None:
        ...
        if normalized is None:
            return None
        if normalized not in ALLOWED_TRANSFERENCIAS_GROUP_FIELDS:
    ...
    def _normalize_metrica(cls, value: Any) -> str:
        normalized = clean_text(value) or "soma_valor"
        if normalized not in ALLOWED_TRANSFERENCIAS_METRICS:
    ...
    def _validate_aggregation(self) -> "AgregarTransferenciasFinanceirasParams":
