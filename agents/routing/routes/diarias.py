...
def _is_diarias_query(normalized_text: str) -> bool:
    return _contains_any(normalized_text, DIARIAS_DOMAIN_KEYWORDS)


def _extract_diarias_filters(normalized_text: str) -> dict[str, object]:
...
def _try_route_diarias_agregacao(normalized_text: str) -> RouteDecision | None:
    if not _is_diarias_query(normalized_text):
        return None
    if not any(
    ...
    filtros = _extract_diarias_filters(normalized_text)
    ...
    return RouteDecision(
        ...
        tags=["scope:public", "domain:diarias", "shape:aggregate"],
        confident=True,
    )
...
def _try_route_diarias_lista(normalized_text: str) -> RouteDecision | None:
    ...
    return RouteDecision(
        ...
        tags=["scope:public", "domain:diarias", "shape:lookup"],
        confident=True,
    )
