...
class DiariasCsvParser:
    ...
    def parse(self, filepath: str) -> list[dict[str, Any]]:
        ...
        categoria = metadata.get("tipo de despesa") or "DIARIAS"
        descricao = metadata.get("descricao_relatorio") or "Diarias"
        ...
        for sequencia, row in enumerate(
            ...
            payload_raw = {
                ...
                "numero_documento": f"DIARIA-{metadata.get('exercicio')}-{sequencia:05d}",
            ...
            try:
                ...
                invalidos += 1
                logger.warning(f"Descartando linha invalida de diarias CSV: {exc}")
                continue
    ...
    def _extract_metadata(
        ...
        if header_index is None:
            raise ValueError(
                f"Cabecalho de diarias CSV nao encontrado no arquivo '{filepath}'."
            )
