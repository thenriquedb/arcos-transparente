"""Modelos ORM de transferencias financeiras e emendas parlamentares."""
...
class TransferenciaFinanceiraMovimento(Base):
    ...
    identificacao: Mapped[Optional[str]] = mapped_column(String(40), nullable=True, index=True)
    unidade_gestora_concessora: Mapped[Optional[str]] = mapped_column(String(255), nullable=True, index=True)
    unidade_gestora_recebedora: Mapped[Optional[str]] = mapped_column(String(255), nullable=True, index=True)
    ...
    programacao_inicial: Mapped[Optional[Decimal]] = mapped_column(Numeric(15, 2), nullable=True)
    data_movimento: Mapped[date] = mapped_column(Date, nullable=False, index=True)
    tipo_movimento: Mapped[Optional[str]] = mapped_column(String(120), nullable=True, index=True)
    valor_movimento: Mapped[Optional[Decimal]] = mapped_column(Numeric(15, 2), nullable=True)
...
class EmendaParlamentar(Base):
    ...
    tipo_emenda: Mapped[Optional[str]] = mapped_column(String(255), nullable=True, index=True)
