...
class Licitacao(Base):
    ...
    __table_args__ = (UniqueConstraint("numero", "data_abertura", name="uq_licitacao_numero_data_abertura"),)
    ...
    vencedores: Mapped[list["VencedorLicitacao"]] = relationship(back_populates="licitacao", cascade="all, delete-orphan")
    instrumentos_contratuais: Mapped[list["InstrumentoContratual"]] = relationship(back_populates="licitacao", cascade="all, delete-orphan")
...
class InstrumentoContratual(Base):
    ...
    __table_args__ = (UniqueConstraint("licitacao_id", "numero_instrumento", name="uq_instrumento_licitacao_numero"),)
...
class MateriaInstrumento(Base):
    ...
    __table_args__ = (UniqueConstraint("instrumento_id", "numero_lote", "numero_item", name="uq_materia_instrumento_lote_item"),)

    id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
    criado_em: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    atualizado_em: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now(), nullable=False)
    instrumento_id: Mapped[int] = mapped_column(ForeignKey("instrumentos_contratuais.id", ondelete="CASCADE"), nullable=False, index=True)
    unidade_gestora: Mapped[Optional[str]] = mapped_column(String(120), nullable=True)
